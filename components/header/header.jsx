...
const Header = (isDark) => {
					...
					subCategory: [
						{
							...
							subLinks: [
								...
								},
								{
									...
									url: '/services/brand-messaging',
								},
								{
									label: 'Visual Identity',
									url: '/services/visual-identity',
								},
								{
									label: 'Data & Metrics Analysis',
									url: '/services/data-metrics-analytics',
								},
								{
									label: 'User Research',
						...
						},
						{
							...
							subLinks: [
								...
								},
								{
									...
									url: '/services/creative-content-services',
								},
								{
									label: 'Motion Graphics & Videos',
									url: '/services/motion-graphics-videos',
								},
								{
									label: 'Campaigns & Marketing',
									url: '/services/campaigns-marketing',
								},
							],
						},
						{
							...
							subLinks: [
								{
									...
									url: '/services/dxp-cms-platforms',
								},
								{
									label: 'Technology Integration',
									url: '/services/technology-integration',
								},
								{
									label: 'Customer Web Development',
									url: '/services/customer-web-development',
								},
								{
									label: 'Authoring & Publishing',
									url: '/services/authoring-publishing',
								},
								{
									label: 'Systems Support',
									url: '/services/systems-support',
								},
							],
						},
					],
	...
	const handleScroll = () => {
		...
		const stickyHeight =
			document.querySelector('#stickyNav .stickyOuter')?.offsetHeight || 0;
		...
		>
			<div className="container">
				<div className={`wrapper flex justify-between lg:items-center`}>
					...
					<div className={`${styles.alignNav} nav__wrap lg:absolute lg:left-0 lg:top-[100%] lg:w-full lg:overflow-y-scroll lg:bg-white transition-max-height duration-700 ease ${isMenuOpen ? 'lg:h-[95vh]' : 'lg:h-0'}`}>
							...
							<nav className="">
								<ul className="lg-up:flex">
									{navData.map((item, index) => (
										<li key={index}
											className={`${styles.menu} ${index != navData.length - 1 ? 'xl-up:pr-[6.6rem] laptop:pr-[5rem]' : ''} ${item.dropLabel}_menu lg:border-b lg:border-black/[0.2]`}
											onMouseEnter={() =>
										...
										>
											<div className="lg:relative">
												...
												<span
													className="lg-up:hidden cursor-pointer absolute top-1/2 -translate-y-1/2 right-0 border-[0.4rem] border-pink rounded-full h-[3.5rem] w-[3.5rem] inline-block mt-0"
											...
											<ul ref={el => subMenuRefs.current[index] = el}
												...
												{item.dropLabel === 'Work' ?
													<li className="container lg:p-[0!important]">
														<div
															className={`lg-up:flex lg:pb-[1.6rem] lg-up:py-[5.4rem] laptop-landscape:h-[70vh] laptop-landscape:overflow-y-scroll ${item.dropLabel === 'About' ? 'lg-up:min-h-[44rem]' : 'lg-up:min-h-[34rem]'}`}
														>

															{isHoverEnabled ? (
																<>
																	<div style={descriptionWidth} className="grow-0 shrink-0 laptop:pr-[25px]">
																		<div className={`${styles.description} max-w-[37rem]`}>
																			{item?.description && (
																				<div>
																					<div className="heading-wrap relative inline-block lg:hidden">
																						<h3 className={`underline-container ${item?.description?.lottiefile} relative inline-block leading-1 pb-[0.8rem] lg:hidden`}>
																							{item?.description?.heading}
																						</h3>
																						<div className={`${item?.description?.work ? 'xl:bottom-[-2rem]' : ''} headlineLottieWrapper img-wrap absolute bottom-[-1rem] w-full h-auto`}>
																							<Lottie
																								options={{
																									loop: false,
																									autoplay: false,
																									animationData: item?.description?.lottiefile,
																									rendererSettings: {
																					...
																					<p className="small mt-[1.4rem] lg:mt-0 lg:pr-[0.1rem]">{item?.description?.subText}</p>
																					<Link data-circle-expand href={item?.description?.url} className="tertiary-btn black mt-[3rem] lg:mt-[2rem]">
																		...
																		{item?.subMenu.map((subMenuItem, subIndex) => (
																			<>
																				{subMenuItem?.subItems && (
																					<div key={subIndex} className="w-[22%] laptop:w-[30%]">
																						<ul className={``}>
																							{subMenuItem.subItems.map((subItem, subItemIndex) => (
																								<li key={subItemIndex} className={`${subMenuItem.subItems.length > 4 ? `lg-up:w-[42%] inline-block ${subItemIndex === 0 ? 'pt-0' : ''} ${subItemIndex === 1 ? 'pt-0' : ''}` : `${subItemIndex === 0 ? 'pt-0' : ''}`} py-[1.5rem] pr-[2rem] lg:w-full lg:pt-[1.3rem] lg:pb-[2.2rem]`}>
																									<Link data-circle-expand href={subItem.url} className={`${styles.subMenuLink} text-[20px] lg:text-[16px] font-bold largedesktop-up:text-[2rem]`}>
																										{subItem.label}
																									</Link>
																								</li>
																							))}
																				...
																				{subMenuItem?.industries && (
																					<div className={`w-[75%] ${styles.industryDivider}`}>
																						...
																						<div className="w-full lg-up:flex gap-x-[1rem] lg-up:mt-[2.4rem]">
																							<div className="lg:w-full">
																								<ul className="lg:mt-[2.3rem]">
																									{subMenuItem?.industries?.col1.subLinks.map((item, itemIndex) => (
																										<li key={itemIndex} className={`pr-[2rem] lg:w-full largedesktop:!text-[20px] !text-[16px] largedesktop-up:text-[2rem] lg:pt-[1.3rem] lg:pb-[2.2rem] lg-up:mb-[1.9rem] ${itemIndex === 0 ? '!mt-0' : ''} ${item.bold === true ? 'font-bold lg-up:mt-[3rem]' : ''}`}>
																											<Link data-circle-expand href={item.url} className={`${styles.subMenuLink}`}>
																												{item.label}
																											</Link>
																										</li>
																									))}
																							...
																							<div className="lg:w-full">
																								<ul className="lg:mt-[2.3rem]">
																									{subMenuItem?.industries?.col2?.subLinks.map((ind, indIndex) => (
																										<li key={indIndex} className={`pr-[2rem] lg:w-full largedesktop:!text-[20px] !text-[16px] largedesktop-up:text-[2rem]lg:pt-[1.3rem] lg:pb-[2.2rem] lg-up:mb-[3rem] ${ind.bold === true ? 'font-bold' : ''}`}>
																											<Link data-circle-expand href={ind.url} className={`${styles.subMenuLink}`}>
																												{ind.label}
																											</Link>
																										</li>
																									))}
																...
																<div className={`flex flex-wrap w-full`}>
																	<div className={`w-full`}>
																		...
																		{item?.subMenu.map((subMenuItem, subIndex) => (
																			<>
																				{subMenuItem?.subItems && (
																					<div key={subIndex} className={`w-full mt-[4.3rem]`}>
																						<ul className={``}>
																							{subMenuItem.subItems.map((subItem, subItemIndex) => (
																								<li
																									key={subItemIndex}
																					...
																					<>
																						{subMenuItem?.industries && (
																							<ul className={``}>
																								...
																								{subMenuItem?.industries?.col1.subLinks.map((item, itemIndex) => (
																									<li key={itemIndex} className={`pr-[2rem] lg:w-full text-[16px] lg:pt-[1.3rem] lg:pb-[2.2rem] lg-up:mb-[1.9rem] ${itemIndex === 0 ? '!mt-0' : ''} ${item.bold === true ? 'font-bold lg-up:mt-[3rem]' : ''}`}>
																										<Link data-circle-expand href={item.url} className="">
																											{item.label}
																										</Link>
																									</li>
																								))}
																					...
																					<>
																						{subMenuItem?.industries && (
																							<div className="w-full">
																								<ul className={``}>
																									{subMenuItem?.industries?.col2?.subLinks.map((ind, indIndex) => (
																										<li key={indIndex} className={`pr-[2rem] w-full text-[16px] pt-[1.3rem] pb-[2.2rem] ${ind.bold === true ? 'font-bold' : ''}`}>
																											<Link data-circle-expand href={ind.url} className="">
																												{ind.label}
																											</Link>
																										</li>
																									))}
																	...
																	{winWidth < 768 && (
																		<div className="w-full inline-block">
																			{item.subMenu?.[1] && (
																				<ul className={``}>
																					<li className={`w-full pr-[2rem] pt-[1.3rem] pb-[2.2rem] text-[16px] font-bold`}>
																						<Link data-circle-expand href=''>
																							{item.subMenu?.[1]?.industries?.col1.subCategoryType}
																						</Link>
																					</li>
																					{item.subMenu?.[1]?.industries?.col1?.subLinks.map((item, itemIndex) => (
																						<li key={itemIndex} className={`w-full pr-[2rem] pt-[1.3rem] pb-[2.2rem] text-[16px] ${item.bold === true ? 'font-bold' : ''}`}>
																							<Link data-circle-expand href={item.url} className="">
																								{item.label}
																							</Link>
																						</li>
																			...
																			{item.subMenu?.[1] && (
																				<ul className={``}>
																					{item.subMenu?.[1]?.industries?.col2?.subLinks.map((ind, indIndex) => (
																						<li key={indIndex} className={`pr-[2rem] w-full text-[16px] pt-[1.3rem] pb-[2.2rem] ${ind.bold === true ? 'font-bold' : ''}`}>
																							<Link data-circle-expand href={ind.url} className="">
																								{ind.label}
																							</Link>
													...
													<li className="container lg:p-[0!important] lg-up:pb-[5.4rem]">
														<div
															className={`lg-up:flex  lg:pb-[1.6rem] laptop-landscape:max-h-[70vh] lg-up:pt-[5.4rem] laptop-landscape:overflow-y-scroll ${item.dropLabel === 'About'
																? 'lg-up:min-h-[44rem]'
																: 'lg-up:min-h-[34rem]'
																}`}
														>
															{isHoverEnabled ? (
																item?.subMenu.map((subMenuItem, subIndex) => (
																	<React.Fragment key={subIndex}>
																							...
																							<div key={descIndex}>
																								<div className="heading-wrap relative inline-block lg:hidden">
																									{desc.about === true ?
																										...
																										>
																											About <span className='relative overflow-visible'>Clear
																												<div
																										...
																										<h3
																											className={`underline-container ${desc.lottiefile} ${desc.approach === true ? '!pb-[0.4rem]' : ''} relative inline-block leading-1 pb-[0.8rem] lg:hidden`}
																										>
																											{desc.heading}
																										</h3>}
																									{desc.about != true &&
																								...
																								<Link
																									data-circle-expand
																									href={desc.url}
																									className="tertiary-btn black mt-[3rem] lg:mt-[2rem]"
																								>
																									{desc.label}
																								</Link>
																							</div>
																		...
																		{(subMenuItem?.subItems || subMenuItem?.subCategory) &&
																			<React.Fragment key={subIndex}>
																				{subMenuItem?.subItems && (
																					<div
																						className={`flex-[28%]`}
																					>
																						<ul className={`${styles.insightsUl} ${item.dropLabel === 'About' || item.dropLabel === 'Insights' ? 'xxl-up:w-[68%] w-[75%]' : ''}`}>
																							{subMenuItem.subItems.map(
																								(subItem, subItemIndex) => (
																									<li
																										key={subItemIndex}
																										className={`${subMenuItem.subItems.length > 4
																				...
																				{subMenuItem?.subCategory &&
																					Array.isArray(subMenuItem.subCategory) && (
																						<div
																							...
																							(desc, descIndex) => (
																								<div key={descIndex}>
																									...
																									<Link
																										data-circle-expand
																										href={desc.url}
																										className="tertiary-btn black mt-[2rem]"
																									>
																										{desc.label}
																									</Link>
																								</div>
