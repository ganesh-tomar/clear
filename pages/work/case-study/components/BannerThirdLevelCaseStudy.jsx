...
export default function BannerThirdLevelCaseStudy({ content }) {
	console.log(content?.linktxt);

	return (
		<section
			className={`banner-third ${content?.dark__overlay ? ' dark__overlay' : ''} ${content?.smallpaddingbottom === true ? 'small-padding-bottom' : ''}  ${content?.bgcolor} overflow-hidden grid__parallax flex items-center ptsp`}
		>
			{content?.vector && (
				<div
			...
			<div className="breadcrumbs w-full">
				<div className="container">
					<ul className={`absolute top-[15%] z-[10] sm:top-[8%]`}>
			...
			<div className="container">
				...
				>
						...
						<div className="area-wrap max-w-[500px] sm:max-w-[100rem] flex flex-wrap mt-[2.9rem] ">
							{
								content?.area?.map((item, index) => {
									return (
										<div key={index} className="content relative w-[50%] max-w-[210px] mb-[1.5rem] mr-[2.8rem] sm:w-[100%] sm:max-w-[100%] sm:mr-[0]">
											<Link
					...
					
					{
						...
						<div className="btn-wrap mt-[6rem] sm:mt-[3rem] link relative inline-block">
							{content?.btnText?.map((item, index) => {
								return (
									// <div className="btn-wrap first:mt-[4.8rem] sm:mt-[3rem]">
										<Link key={index}
											href={`${item?.linkUrl}`} 
											className=" text-[#FA198C] medium font-bold relative block"
										>
											{item?.linktxt}
										</Link>
									// </div>
					...
					content?.logoWithDescription == true ? 
						<div className="logoWithDescription mt-[30px]">
							<div className="row flex flex-wrap items-center">
								<div className="logoWrap w-[150px] lg:w-[100px] sm:w-full">
								...
								<div className="contentWrap w-[calc(100%-150px)] ml-[20px] sm:w-full sm:mt-[20px] sm:ml-0">
									{
										content?.logoWithText?.map((item, index) => {
											return (
												<div className="item mb-[20px]" key={index}>
													<h4>{item?.title}</h4>
													<p>{item?.paragraph}</p>
												</div>
											)
