    ...
    return (
        <section className={`bladewiththreeimg height__full-nav imageSlider relative before:hidden after:hidden xldesktop-up:min-h-[115vh] phablet:min-h-[370px] tablet:min-h-[500px] laptopsmall:min-h-[700px] laptopSmallMid:min-h-[700px] desktopsmallmid:min-h-[740px] laptopbig:min-h-[800px] laptopmid:min-h-[800px] desktopMidUp:min-h-[850px] ipad:min-h-[645px] sm:!min-h-[220px] no-padding overflow-visible ${style.bladewiththreeimg} ${content?.bgColor} ${content?.sectionClass}`}>
            <div className="wrap h-full w-full absolute top-0 lg:h-full sm:flex sm:items-center">
                ...
                <div className={`image-wrapper relative w-full h-full bottom-0 left-0 z-33 laptopsmall:h-[120%] tablet:h-[118%] phablet:h-[118%] ${content?.sectionClass === 'revealAnimation' ? 'sm:h-[calc(100%+30%)] tablet:mt-[-30px] phablet:mt-[-30px]' : 'sm:h-full'}`}>
                    <Image src={content?.imageUrl} width={1920} height={1920} className='elementToBeAnimate w-full object-contain phablet:object-cover tablet:object-cover laptopsmall:object-cover tablet:h-full laptopsmall:h-full phablet:h-full sm:object-cover sm:h-[120%]' alt={content?.imageAlt} />
                </div>
            </div>
