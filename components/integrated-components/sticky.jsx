...
function Sticky(data) {
    ...
    return (
        <>
            {winWidth > 991 ? (
                <section
                    className={`${data.classes} ${sectionPadding} ${StickyStyle.mainSticky} block sticky py-0 z-[9] transition-all duration-300 shadow-bottom-white-shadow `}
                    style={Stickytop}
                    id={data.id}
                >
                                ...
                                {data?.items && data?.items.map((data, index) => {
                                    return (
                                        ...
                                        >
                                            <Link
                                                href={`#${data.sectionId}`}
                                                aria-label={`Navigate to ${data.text}`}
                                                className={`text-black inline-block py-[2rem] ${visibleSections[0] === data.sectionId
                                                    ? `${StickyStyle.activated} activated`
                                                    : ""
                                                    }transition-all hover:text-pink ease-in-out`}
                                                onClick={(e) =>
                                                    handleStickyClick(e, data.sectionId, index, data.sectionId, 50)
                                                }
                                            >
                                                {data.text}
                                            </Link>
