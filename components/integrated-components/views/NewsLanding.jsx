...
import { GET_CONTENT_TYPE, GET_ALL_NEWS } from '../../../graphql/news';
...
const NewsLanding = () => {
	...
	useEffect(() => {
		if (contentTypes.data) {
			setContentTypeData(contentTypes?.data?.customContentTypes?.nodes);
		}
	}, [contentTypes?.data]);
	useEffect(() => {
		if (allNews.data) {
			setNewsData(allNews?.data?.allClientNews?.nodes);
		}
	}, [allNews?.data]);
	...
	const handleInputChange = (e) => {
		...
		if (selectedTopic.length !== 0) {
			filtered = topicData.filter((item) => {
				const typename = item?.terms?.nodes?.[0]?.__typename || '';
				const title = item?.title || '';

				return (
					typename.toLowerCase().includes(value.toLowerCase()) ||
	...
	const Topicfetchhandler = (index, tabName, val) => {
		...
		if (selectedyear == '' && selectedyear.length == 0) {
			filteredData = newsData.filter((item) =>
				item.terms.nodes.some((node) => node.termTaxonomyId === val),
			);
	...
	const yearfetchhandler = (index, val) => {
		...
		if (selectedTopic.length === 0) {
			...
			temp = newsData.filter((item) =>
				item.terms.nodes.some((node) => node.name === selectedTopic),
			);
			// console.log(temp);
						...
						<div className="result-wrap">
							...
							<div className="mini-wrap-case flex flex-wrap justify-between mt-0 mb-[2.8rem]">
								<div className="mini-heading">
									<p className="small">{newsData.length} results </p>
								</div>
							...
							<div className="filter-block border-t-[0.1rem] border-[#989898] ">
								...
								>
									<ul className="pb-[3rem]">
										{contentTypeData.map((tab, index, name) => (
											<li
												...
												>
													<div className="circle-box-wrap large mr-[2rem] pointer-events-none inline top-[0.5rem] relative">
														<input
															...
															id={`topic-${index}`}
															name={tab.name}
															checked={selectedTopic == tab.name}
						...
						<div className="cardswrapper flex flex-wrap w-[calc(100%+3rem)] ml-[-1.5rem] sm:w-full sm:ml-0">
							{FilterData.length != 0 ? (
								FilterData.slice(startIndex, endIndex).map((item, index) => {
									...
									return (
										<div
											className={`overlay ${item?.clientNews.backgroundColor
												? item.clientNews.backgroundColor
												: ''
