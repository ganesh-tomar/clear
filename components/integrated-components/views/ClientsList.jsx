						...
						>
							...
							<ul className="link-wrap-case">
								{clientTypeData.map((item, index) => (
									<li
										key={index} dataval={item.termTaxonomyId}
										...
										data-cursor-expand="true"
									>
										{item.children.nodes.length > 0 ? <span className='lg:!cursor-pointer drop absolute top-[2rem] right-[1.5rem] w-[5rem] h-[5rem]  z-[10]' onClick={(e) => listToggleHandler(e, index)}></span> : ''}

										<div className='lg:!cursor-pointer categoty-wrap z-[5] py-[3rem] w-full medium bold capitalize flex items-center' onClick={(e) => clickHandler(item.termTaxonomyId, e)}>
											<div className="check-box-wrap large mr-[2.1rem] pointer-events-none">
												<input
													...
													name=""
													id={`service-${index}`}
													checked={checkedServicesVal.includes(item.termTaxonomyId)}
												/>
															...
															<li
																...
																onClick={() =>
																	SubCategoryClickHandler(
																		elem.name,
																		elem.parent.node.termTaxonomyId
																	)
																}
															>
					...
					<div className="logo-block w-[71%] pt-[0.3rem] pl-[5.5rem] lg:w-full lg:pl-0 lg:pt-0 lg:mt-[3rem]">
						...
						<div className="logo-wrapper w-full relative flex overflow-hidden flex-wrap items-center">
							{filteredData.map((item, index) => (
								<div className={`${item.clients.externalLink ? 'area' : ''} ${style.clientCard} client-card flex flex-wrap items-center w-[25%] laptop:w-[33.33%] h-[20rem] tablet:h-[18rem] phablet:h-[18rem] sm:h-[16rem] laptop:h-[29rem] phablet:w-[33.33%] sm:w-1/2`} key={index}>
									<span className={`absolute w-full h-full left-0 top-0 bottom-dotted-border right-dotted-border`}></span>
									{item.clients.externalLink ? (<Link href={item.clients.externalLink} className='redirect'>.</Link>) : ''}
									{item.clients.logo && (
