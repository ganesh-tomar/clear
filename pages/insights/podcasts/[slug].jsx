...
// import { GET_PODCAST_BY_SLUG } from '../../../graphql/posts';
...
//   try {
//     // Fetch the specific podcast content by slug
//     const { data: podcastData } = await apolloClient.query({
//       query: GET_PODCAST_BY_SLUG,
//       variables: { slug },
//     });

//     // If podcast is not found, return 404
//     if (!podcastData.podcastBy) {
//       return {
//         notFound: true,
//       };
//     }
...
//     // Exclude the current post (podcast) from the list of latest posts
//     const recentPosts = latestPosts.filter((latestPost) => latestPost.slug !== slug).slice(0, 3);

//     // Return the podcast data and recent posts as props
//     return {
//       props: {
//         podcast: podcastData.podcastBy,
...
// const PodcastDetail = ({ podcast, recentPosts, formID }) => {
...
//             <meta name="description" content={`podcast created by ${podcast.author?.node?.name}`} />
//           </Head>
...
//                       src={podcast?.featuredImage?.node?.mediaItemUrl || '/default-image.jpg'}
//                     />
//                   </div>
//                 }
...
//                         <h1 className="h2 text-white">{podcast?.title}</h1>
//                     </div>
//                 </div>
//             </section>
...
//                       <Image
//                         src={podcast?.author?.node?.userProfile?.userAvatar?.sourceUrl}
//                         width={2000}
//                         height={2000}
//                         className="w-full h-full object-contain rounded-full"
//                         alt={podcast?.author?.node?.userProfile?.userAvatar?.altText}
//                       />
//                     </div>
//                     <div className="authorDesc w-[calc(100%-12rem)]">
//                     <p>By <Link href={`/author/${podcast?.author?.node?.name}`} className="underline">{podcast?.author?.node?.firstName} {podcast?.author?.node?.lastName}</Link></p>
...
//                         <Link className="redirect" href={`https://www.linkedin.com/shareArticle?mini=true&url=${fullUrl}&title=${podcast.title}&summary=&source=${fullUrl}`} target="_blank">
//                           .
//                         </Link>
//                         <svg
//                           width="40"
...
//                         <Link className="redirect"  href={`https://www.facebook.com/sharer.php?u=${fullUrl}&t=${podcast.title}`} target="_blank">
//                           . 
//                         </Link>
//                         <svg
//                           width="40"
