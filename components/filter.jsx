					...
					<div className="logo-block w-[71%] pt-[0.3rem] pl-[5.5rem] lg:w-full lg:pl-0 lg:pt-0 lg:mt-[3rem]">
						...
						<div className="logo-wrapper w-full relative flex overflow-hidden flex-wrap items-center">
							{filteredData.map((item, index) => (
								<div className={`${item.hasOwnProperty("ctaLink") ? 'area' : ''} ${style.clientCard} client-card flex flex-wrap items-center w-[25%] laptop:w-[33.33%] h-[20rem] tablet:h-[18rem] phablet:h-[18rem] sm:h-[16rem] laptop:h-[29rem] phablet:w-[33.33%] sm:w-1/2`} key={index}>
									<span className={`absolute w-full h-full left-0 top-0 bottom-dotted-border right-dotted-border`}></span>
									{item.hasOwnProperty("ctaLink") ? (<Link href="" className='redirect'>.</Link>) : ''}
