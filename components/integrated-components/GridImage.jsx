...
const ImageGrid = ({ items, halfscreen, smallscreen }) => {
	...
	return (
		<section
			className={`imagegrid no-padding ${halfscreen ? 'height__full-nav grid lg:min-h-[60rem] sm:!min-h-[30rem]' : ''} ${
				!halfscreen && isSmScreen ? 'sm:!h-auto' : 'h-[54.5rem] lg:h-[30.5rem]'
			}`}
		>
			{items.map((item, index) => {
				...
				return (
					<div
						key={index}
						className={`bg-img absolute flex flex-wrap items-center sm:!h-[25rem] ${
							smallscreen ? 'sm:min-h-[25vh]' : ''
						}`}
						style={{
							...styles,
