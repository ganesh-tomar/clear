...
// import { GET_INFOGRAPHIC_BY_SLUG } from '../../../graphql/posts';
...
//   try {
//     // Fetch the specific infographic content by slug
//     const { data: infographicData } = await apolloClient.query({
//       query: GET_INFOGRAPHIC_BY_SLUG,
//       variables: { slug },
//     });

//     // If infographic is not found, return 404
//     if (!infographicData.infographicBy) {
//       return {
//         notFound: true,
//       };
//     }
...
//     // Exclude the current post (infographic) from the list of latest posts
//     const recentPosts = latestPosts.filter((latestPost) => latestPost.slug !== slug).slice(0, 3);

//     // Return the infographic data and recent posts as props
//     return {
//       props: {
//         infographic: infographicData.infographicBy,
...
// const InfographicDetail = ({ infographic, recentPosts, formID }) => {
...
//         <meta name="description" content={`infographic created by ${infographic.author?.node?.name}`} />
//       </Head>

//       {/* Banner Section */}
//       <section className="insights-banner dark__overlay grid__parallax overflow-hidden  lg-up:min-h-[80rem] lg-up:justify-between flex items-center bg-black">
//         {infographic?.featuredImage &&
//           <div className="bg-img !w-[100%] !z-0">
//             <Image
//               alt={infographic?.featuredImage?.node.altText}
//               width="1600"
//               height="1090"
//               className="object-bottom xxl-down:object-center"
//               src={infographic?.featuredImage?.node?.mediaItemUrl || '/default-image.jpg'}
//             />
//           </div>
//         }
...
//             <h1 className="h2 text-white">{infographic?.title}</h1>
//           </div>
//         </div>
//       </section>
...
//                       <Image
//                         src={infographic?.author?.node?.userProfile?.userAvatar?.sourceUrl}
//                         width={2000}
//                         height={2000}
//                         className="w-full h-full object-contain rounded-full"
//                         alt={infographic?.author?.node?.userProfile?.userAvatar?.altText}
//                       />
//                     </div>
//                     <div className="authorDesc w-[calc(100%-12rem)]">
//                       <p>By <Link href={`/author/${infographic?.author?.node?.name}`} className="underline">{infographic?.author?.node?.firstName} {infographic?.author?.node?.lastName}</Link></p>
//                       <p className="small !m-0">
//                         {new Date(infographic?.date).toLocaleDateString('en-US', {
//                           year: 'numeric',
//                           month: 'long',
//                           day: 'numeric',
//                         })}
...
//                     {infographic.tags.nodes.map((tag) => (
//                       <Link href={tag.uri}>
...
//                       <Link className="redirect" href={`https://www.linkedin.com/shareArticle?mini=true&url=${fullUrl}&title=${infographic.title}&summary=&source=${fullUrl}`} target="_blank">
//                         .
//                       </Link>
//                       <svg
//                         width="40"
...
//                       <Link className="redirect" href={`https://www.facebook.com/sharer.php?u=${fullUrl}&t=${infographic.title}`} target="_blank">
//                         .
//                       </Link>
//                       <svg
//                         width="40"
...
//                 </div>
//               )}
//               <div dangerouslySetInnerHTML={{ __html: infographic?.content }} />
