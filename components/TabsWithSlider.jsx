            ...
            {isWideScreen ? (
                <section className={`${style.tabSlider} tabSlider tabSliderFullWidth bg-gray no-padding top-0 left-0 z-[8] block ${data.classes} ${data?.padding.join(' ')}`} id={data.id}>
                    <div className="wrapper flex w-full">
                        <div className="tabListOuter relative z-10 flex flex-col justify-between w-1/2 lg-up:pb-[14.8rem] bg-gray containerPadding pt-[7.5%] xxl-up:pt-[70px]">
                        ...
                        <div className="tabContentOuter overflow-x-hidden dark__overlay relative w-1/2">
                            ...
                            {data.items.map((item, i) => {
                                return (
                                    <div key={i} className={`tabContent ${style.tabContent} ${i === activeIndex ? style.activeContent : ''} absolute top-0 right-0 w-full h-full transition-transform duration-500 ease-linear lg-up:pl-[10.5rem] lg-up:pb-[14.8rem] containerPadding overflow-hidden pt-[15%] xxl-up:pt-[70px]`}>
                        ...
                        <div className="accordionWrap">
                            {accordionItems.map((item, index) => (
                                <div data-cursor-expand="true" data-cursor-icon={activeIndex === index ? "iconMinus" : "iconPlus"} key={index} className={`accordion-item relative  ${activeIndex === index ? "" : "border-b border-black"}`} onClick={() => toggleAccordion(index)}>
                                    ...
                                    <div
                                        className={`accordioncontent relative max-w-[123.9rem] overflow-y-clip overflow-visible transition-max-height duration-300 ease-in-out ${activeIndex === index ? 'max-h-[250rem] open' : 'max-h-0 invisible'
